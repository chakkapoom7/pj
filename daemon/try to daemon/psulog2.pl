...
sub getInitialValue{
  open(File,"/etc/psulog/config.cnf");
...
####################### initial value ############################
	...
	$interval = $init[3];
	$sleepMainLoop = $interval


	print "time interval = $interval min\n";
	print "switch address = $switch_v6address\n";

	logEntry("time interval = $interval min\n");
	logEntry("switch address = $switch_v6address\n");
